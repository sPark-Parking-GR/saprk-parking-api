...
const hhmm = (point: { hour: number; minute: number }): string => `${pad(point.hour)}:${pad(point.minute)}`
...
function convertOpeningHours(place: Place): CanonicalOpeningHours {
  ...
  const [first] = periods
  if (periods.length === 1 && first && !first.close && first.open.hour === 0 && first.open.minute === 0) {
    return { is24h: true }
  }
...
function deriveRules(place: Place): CanonicalRule[] {
  ...
  if (place.businessStatus) rules.push({ ruleKey: 'google:business_status', ruleValue: place.businessStatus })
  if (place.types.length > 0) rules.push({ ruleKey: 'google:types', ruleValue: place.types.join(',') })
  return rules
}
