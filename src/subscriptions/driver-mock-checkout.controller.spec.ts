...
import type { DriverSubscriptionEventsService } from './driver-subscription-events.service'
...
interface Harness {
  controller: DriverMockCheckoutController
  provider: {
...
function build(
  ...
  const provider = {
    ...
    completeCheckoutSession: jest.fn().mockReturnValue({
      ...
      eventCreatedAt: new Date('2026-08-27T12:00:00Z'),
      raw: {},
  ...
  const controller = new DriverMockCheckoutController(
    ...
    events as unknown as DriverSubscriptionEventsService,
  )
 ...
 */
describe('DriverMockCheckoutController — refuses to exist on a real provider', () => {
...
describe('DriverMockCheckoutController — the page', () => {
  it('renders the plan, price and both actions, and relaxes only the CSP it needs', () => {
    ...
    expect(sent.body).toContain('plus')
    expect(sent.body).toContain('4.99')
...
describe('DriverMockCheckoutController — confirm and cancel', () => {
  /**
   ...
   * shortcut around it.
   */
  it('feeds the provider’s event into the real event service, then returns to the app', async () => {
  ...
  it('cancels the session and returns to the app’s cancel route', () => {
